...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <!-- page head start-->
        <div class="page-head">
            <h3 class="m-b-less">Report PO Ledger</h3>
            <div class="state-information">
                <ol class="breadcrumb m-b-less bg-less">
                    <li><a href="<?=base_url('admin/dashboard');?>">Home</a></li>
                    <li class="active"> Report PO Ledger</li>
                </ol>
            </div>
        </div>
        <!-- page head end-->
                        ...
                        <div class="panel-body">
                            <form id="form_add_customer_invoice" method="post" action="" target="_blank" enctype="multipart/form-data" class="cmxform form-horizontal tasi-form">
                                <div class="form-group ">
                                    ...
                                    <div class="col-lg-3">
                                      <label for="po_id" class="control-label">PO Number</label>
                                      <select id="po_id" name="po_id" class="select2 form-control">
                                        <option value="">Select PO Number</option>
                                        <?php 
                                        $purchase_orders = $po_report['purchase_order'];
                                        foreach($purchase_orders as $pos){ ?>
                                        <option value="<?=$pos->po_id?>" <?php if($pos->po_id == $po_report['po_id']){?> selected="selected" <?php } ?>><?=$pos->po_number?></option>
                                        <?php } ?>
                                      </select>
                                    </div>
                                    
                                    <div class="col-lg-3">
                                      <label for="pod_id" class="control-label text-danger">CN Number*</label>
                                      <select id="pod_id" name="pod_id[]" class="select2 form-control" multiple>
                                        <option value="">Select CN Number</option>
                                        <?php 
                                        if(isset($po_report['purchase_order_detail'])){
                                        $purchase_order_detail = $po_report['purchase_order_detail'];
                                        foreach($purchase_order_detail as $pods){ ?>
                                        <option value="<?=$pods->pod_id?>"><?=$pods->consignement_number?>/<?=$pods->size_in_text?></option>
                                        <?php }} ?>
                                      </select>
                                    </div>
